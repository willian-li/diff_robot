...
import os
import example_2.hardware.base_control as base_control
import tf
import time
import sys
import math
...
class BaseControl:
    def __init__(self):
        ...
        #Get params
        self.baseId = base_control.get_param('~base_id','base_footprint')
        self.odomId = base_control.get_param('~odom_id','odom')
        self.device_port = base_control.get_param('~port','/dev/ttyUSB0')
        self.baudrate = int(base_control.get_param('~baudrate','115200'))
        self.odom_freq = int(base_control.get_param('~odom_freq','50'))
        self.odom_topic = base_control.get_param('~odom_topic','/odom')
        self.battery_topic = base_control.get_param('~battery_topic','battery')
        self.battery_freq = float(base_control.get_param('~battery_freq','1'))
        self.cmd_vel_topic= base_control.get_param('~cmd_vel_topic','/cmd_vel')
        self.ackermann_cmd_topic = base_control.get_param('~ackermann_cmd_topic','/ackermann_cmd_topic')
        self.pub_imu = bool(base_control.get_param('~pub_imu',False))
        if(self.pub_imu == True):
            self.imuId = base_control.get_param('~imu_id','imu')
            self.imu_topic = base_control.get_param('~imu_topic','imu')
            self.imu_freq = float(base_control.get_param('~imu_freq','50'))
            if self.imu_freq > 100:
                self.imu_freq = 100
        self.pub_sonar = bool(base_control.get_param('~pub_sonar',False))
        self.sub_ackermann = bool(base_control.get_param('~sub_ackermann',False))
        self.boardcast_odom_tf = bool(base_control.get_param('~boardcast_odom_tf',True))

        #define variable
        self.current_time = base_control.Time.now()
        ...
        self.last_cmd_vel_time = base_control.Time.now()
        self.last_ackermann_cmd_time = base_control.Time.now()
        ...
        try:
            ...
            base_control.loginfo("Opening Serial")
            try:
                if self.serial.in_waiting:
                    self.serial.readall()
            except:
                base_control.loginfo("Opening Serial Try Faild")
                pass
        except:
            base_control.logerr("Can not open Serial"+self.device_port)
            self.serial.close
            sys.exit(0)
        base_control.loginfo("Serial Open Succeed")
        ...
        if(('NanoCar' in base_type) & (self.sub_ackermann == True)):
            ...
            self.sub = base_control.Subscriber(self.ackermann_cmd_topic,AckermannDriveStamped,self.ackermannCmdCB,queue_size=20)
        else:
            self.sub = base_control.Subscriber(self.cmd_vel_topic,Twist,self.cmdCB,queue_size=20)
        self.pub = base_control.Publisher(self.odom_topic,Odometry,queue_size=10) #odom publisher
        self.battery_pub = base_control.Publisher(self.battery_topic,BatteryState,queue_size=3)
        if self.boardcast_odom_tf:
            self.tf_broadcaster = tf.TransformBroadcaster()
        self.timer_odom = base_control.Timer(base_control.Duration(1.0/self.odom_freq),self.timerOdomCB)
        self.timer_battery = base_control.Timer(base_control.Duration(1.0/self.battery_freq),self.timerBatteryCB)  
        self.timer_communication = base_control.Timer(base_control.Duration(1.0/1000),self.timerCommunicationCB)

        #inorder to compatibility old version firmware,imu topic is NOT pud in default
        if(self.pub_imu):            
            self.imu_pub = base_control.Publisher(self.imu_topic,Imu,queue_size=10)
            self.timer_imu = base_control.Timer(base_control.Duration(1.0/self.imu_freq),self.timerIMUCB) 
        self.getVersion()
    ...
    def cmdCB(self,data):
        ...
        try:
            ...
            base_control.logerr("Vel Command Send Faild")
        self.serialIDLE_flag = 0
    ...
    def getVersion(self):
        ...
        try:
            ...
            base_control.logerr("Get Version Command Send Faild")
        self.serialIDLE_flag = 0 
    ...
    def getSN(self):
        ...
        try:
            ...
            base_control.logerr("Get SN Command Send Faild")
        self.serialIDLE_flag = 0  
    ...
    def getInfo(self):
        ...
        try:
            ...
            base_control.logerr("Get info Command Send Faild")
        self.serialIDLE_flag = 0               
    ...
    def timerOdomCB(self,event):
        ...
        try:
            ...
            base_control.logerr("Odom Command Send Faild")
        self.serialIDLE_flag = 0   
        #calculate odom data
        Vx = float(ctypes.c_int16(self.Vx).value/1000.0)*0.856
        Vy = float(ctypes.c_int16(self.Vy).value/1000.0)*0.856
    ...
    def timerBatteryCB(self,event):
        ...
        try:
            ...
            base_control.logerr("Battery Command Send Faild")
        self.serialIDLE_flag = 0
        msg = BatteryState()
        msg.header.stamp = self.current_time
    ...
    def timerIMUCB(self,event):
        ...
        try:
            ...
            base_control.logerr("Imu Command Send Faild")

        self.serialIDLE_flag = 0
        msg = Imu()
        msg.header.stamp = base_control.Time.now()
                ...
                if length > 1 :
                    if self.Circleloop.get_front_second() <= self.Circleloop.get_queue_length():
                        ...
                        if(databuf[3] == 0x04):
                            ...
                            base_control.loginfo(version_string)
                        elif(databuf[3] == 0xf4):
                            sn_string = "SN:"
                            for i in range(4,16):
                                sn_string = "%s%02x"%(sn_string,databuf[i])
                            base_control.loginfo(sn_string)                            
                            ...
                            base_control.loginfo(info_string)
                        else:
                            pass
                else:
                    pass
...
if __name__=="__main__":
    try:
        base_control.init_node('base_control',anonymous=True)
        if base_type != None:
            base_control.loginfo('%s base control ...'%base_type)
        else:
            base_control.loginfo('base control ...')
            base_control.logerr('PLEASE SET BASE_TYPE ENV FIRST')
